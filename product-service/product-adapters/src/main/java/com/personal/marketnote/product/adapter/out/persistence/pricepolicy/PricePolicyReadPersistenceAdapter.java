...
public class PricePolicyReadPersistenceAdapter implements FindPricePoliciesPort {
    ...
    @Override
    public List<PricePolicyWithOptions> findByProductId(Long productId) {
        List<PricePolicyJpaEntity> policies = pricePolicyJpaRepository.findAll().stream()
                ...
                .toList();
        List<PricePolicyWithOptions> results = new ArrayList<>();
        for (PricePolicyJpaEntity pp : policies) {
            ...
            results.add(new PricePolicyWithOptions(
                    pp.getId(),
                    pp.getPrice(),
                    pp.getDiscountPrice(),
